...
`kaishular` is designed to make life easy by providing a basic framework
...
kaishular also comes prepackaged with the most popular design frameworks
...
The principal goal of `kaishular` is to set projects up for long-term
success.  So `kaishular` tries to follow best practices everywhere it can.
These are:
...
- It should be responsive to evidence. Community feedback is therefore crucial
  to the success of `kaishular`.

But `kaishular` is not an example of an AngularJS app: this is a
...
getting `kaishular` up and running. Though `kaishular` is really simple
...
`kaishular` uses [Grunt](http://gruntjs.org) as its build system, so
...
```sh
$ git clone git://github.com/joshdmiller/kaishular my-project-name
$ cd my-project-name
```
...
There are many Bower packages used by `kaishular`, like Twitter Bootstrap
...
Technically, `kaishular` is now ready to go.
...
`kaishular`. What follows in this section is a quick introduction to the
...
`kaishular` also includes [Live Reload](http://livereload.com/), so you no
...
default with `kaishular`) you need to tell Live Reload to allow it. Go to
...
See the [issues list](http://github.com/joshdmiller/kaishular/issues). And
